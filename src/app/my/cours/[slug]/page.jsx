  ...
  return (
    <div className="lg:px-28 px-8 2xl:px-80 py-8 flex gap-7 flex-col">
      ...
      <div className="flex flex-col gap-3">
        ...
        <p className="text-slate-700 md:text-sm text-xs">
          Encadré par: {prof.firstname} {prof.lastname}
        </p>
      </div>
      ...
      {role === "teacher" && (
        <div className="inline-flex gap-3">
          <Button className="w-40" href={`/my/prof/AddChapitre/${slug}`}>
            Ajouter un chapitre
          </Button>
          <Button variant="outline" className="w-40">
        ...
        <main className="lg:w-[62%] w-full text-left">
          <Accordion type="single" collapsible className="w-full">
            <AccordionItem value="item-1">
              ...
              <AccordionContent>{objectif}</AccordionContent>
            </AccordionItem>
            <AccordionItem value="item-2">
              <AccordionTrigger>
                <h2 className="font-bold md:text-xl text-lg">Plan du Cours</h2>
              </AccordionTrigger>
              <AccordionContent>
                    ...
                    <li key={chapitre.num}>
                      ...
                      <ul className="list-disc p-1 pl-5 ">
                        {chapitre.elements.map((element, index) => (
                          <li className="my-1 text-xs" key={index}>
                            {element}
                          </li>
                        ))}
                      </ul>
              ...
              <AccordionItem value={"item-" + (index + 3)}>
                <AccordionTrigger>
                  <h2 className="font-bold md:text-lg text-left">
                    Chapitre {chapitre.num}: {chapitre.title}
                  </h2>
                </AccordionTrigger>
                <AccordionContent>
